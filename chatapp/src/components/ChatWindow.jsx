...
function ChatWindow({ selectedUser, currentUser }) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:5000/api/users/messages/${selectedUser._id}`, {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/users/messages', message, {
        headers: { Authorization: `Bearer ${token}` }
      });
